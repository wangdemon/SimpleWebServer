...
void ServerHttp::requestHandler(QTcpSocket *socket, const QString &method, const QString &path,
                                const QStringList &cookies, const QByteArray &requestData)
{
    ...
    QByteArray response = createHeader(path,data.size(),cookies);
...
void ServerHttp::readDataFromSocket()
{
    ...
    foreach (QString line, list)
    {
        ...
        else if(line.contains("Cookie:"))
        {
            ...
            {
                QRegularExpressionMatch match = it.next();
                cookies.append(match.captured(1) + "=" + match.captured(2));
            }
        }
    }

    if(receivers(SIGNAL(request(QTcpSocket*,QString,QString,QStringList,QByteArray))) != 0)
        emit request(socket,method,path,cookies,requestData);
    else requestHandler(socket,method,path,QStringList(),requestData);
}
